...
test('POST /boardMembersMatrixDataPoints 201 (user)', async () => {
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ access_token: userSession, datapointId: 'test', companyId: 'test', boardMemberName: 'test', year: 'test', fiscalYearEndDate: 'test', response: 'test', status: 'test' })
...
test('PUT /boardMembersMatrixDataPoints/:id 401 (user) - another user', async () => {
  const { status } = await request(app())
    .put(`${apiRoot}/${boardMembersMatrixDataPoints.id}`)
    .send({ access_token: anotherSession, datapointId: 'test', companyId: 'test', boardMemberName: 'test', year: 'test', fiscalYearEndDate: 'test', response: 'test', status: 'test' })
  expect(status).toBe(401)
})
